...
local BASE_MOVEMENT_SPEED = 7 --global not present in wotlk, define it here as part of the backport
...
local functionCache = {} 
CNDT.functionCache = functionCache
...
function CNDT:PLAYER_TALENT_UPDATE()
	for tab = 1, GetNumTalentTabs() do
		for talent = 1, GetNumTalents(tab) do
			...
			if lowr then
				Env.TalentMap[lowr] = rank or 0
			end
		end
	end
end
function CNDT:PLAYER_LOGIN()
...
if pclass == "SHAMAN" then
	...
	totemtex = {
		TMW_GetSpellTexture(8227),	-- flametongue
		TMW_GetSpellTexture(10408),	-- stoneskin 78222
		TMW_GetSpellTexture(5675),	-- mana spring
		TMW_GetSpellTexture(3738),	-- wrath of air
	}
end
...
do -- STANCES
	local Stances = {
		...
		{class = "PALADIN", 	id = 48943}, 	-- Shadow Resistance Aura
		{class = "PALADIN", 	id = 19888}, 	-- Frost Resistance Aura
		{class = "PALADIN", 	id = 27153}, 	-- Fire Resistance Aura
...
Env = {
	...
	TMW_GetPrimaryTalentTree = TMW_GetPrimaryTalentTree,
...
-------------------------------------resources
	{ -- health
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_HEALTH", c.Unit, "UNIT_MAXHEALTH", c.Unit
		end,
	},
	--{ -- primary resource
	...
	{ -- mana
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MANA", c.Unit, "UNIT_MAXMANA", c.Unit
		end,
	},
	{ -- energy
		text = ENERGY,
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_ENERGY", c.Unit, "UNIT_MAXENERGY", c.Unit
		end,
	},
	{ -- rage
		text = RAGE,
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_RAGE", c.Unit, "UNIT_MAXRAGE", c.Unit
		end,
	},
	{ -- focus
		text = FOCUS,
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_FOCUS", c.Unit, "UNIT_MAXFOCUS", c.Unit
		end,
	},
	{ -- runic power
		text = RUNIC_POWER,
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_RUNIC_POWER", c.Unit, "UNIT_MAXRUNIC_POWER", c.Unit
		end,
	},
	...
	{ -- soul shards
		...
		tcoords = standardtcoords,
		funcstr = [[GetItemCount(6265, nil, 1) c.Operator c.Level]],
		events = function(c)
	...
	{ -- abs mana
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MANA", c.Unit
		end,
	},
	{ -- abs energy
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_ENERGY", c.Unit
		end,
	},
	{ -- abs rage
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_RAGE", c.Unit
		end,
	},
	{ -- abs focus
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_FOCUS", c.Unit
		end,
	},
	{ -- abs runic power
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_RUNIC_POWER", c.Unit
		end,
	},
	...
	{ -- max mana
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MAXMANA", c.Unit
		end,
	},
	{ -- max energy
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MAXENERGY", c.Unit
		end,
	},
	{ -- max rage
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MAXRAGE", c.Unit
		end,
	},
	{ -- max focus
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MAXFOCUS", c.Unit
		end,
	},
	{ -- max runic power
		...
		events = function(c)
			return CNDT:IsUnitEventUnit(c.Unit), "UNIT_MAXRUNIC_POWER", c.Unit
		end,
	},
...
-------------------------------------player status/attributes
	...
	{ -- stance
		...
		unit = PLAYER,
		icon = function()
			return firststanceid and TMW_GetSpellTexture(firststanceid)
		end,
		tcoords = standardtcoords,
	...
	{ -- talent tree
		...
		texttable = function(i) 
		    return select(1, GetTalentTabInfo(i)) 
		end,
		unit = PLAYER,
		icon = function() 
		    return select(2, GetTalentTabInfo(1)) 
		end,
		tcoords = standardtcoords,
		funcstr = [[TMW_GetPrimaryTalentTree() c.Operator c.Level]],
		events = function(c)
			return "PLAYER_TALENT_UPDATE", "ACTIVE_TALENT_GROUP_CHANGED"
		end,
	},
	{ -- points in talent
		...
		useSUG = "talents",
		icon = function() 
		    return select(2, GetTalentInfo(1, 1)) 
		end,
		tcoords = standardtcoords,
		funcstr = function(c)
		    local name = c.Name and strlowerCache[c.Name]
		    return [[(TalentMap["]].. name .. [["] or 0) c.Operator c.Level]]
		end,	
		events = function(c)
	...
	{ -- pet autocast
		...
		unit = PET,
		name = function(editbox) 
		    TMW:TT(editbox, "CONDITIONPANEL_AUTOCAST", "CNDT_ONLYFIRST") 
			editbox.label = L["SPELLTOCHECK"] 
		end,
		useSUG = true,
...
-------------------------------------spells/items
	...
	{ -- item in bags
		...
		name = function(editbox) 
		    TMW:TT(editbox, "ITEMINBAGS", "CNDT_ONLYFIRST") 
			editbox.label = L["ITEMTOCHECK"] 
		end,
		useSUG = "item",
		unit = false,
	...
	{ -- casting
		...
		tcoords = standardtcoords,
		name = function(editbox) 
		    TMW:TT(editbox, "CONDITIONPANEL_CASTTOMATCH", "CONDITIONPANEL_CASTTOMATCH_DESC") 
			editbox.label = L["CONDITIONPANEL_CASTTOMATCH"] .. " " .. L["ICONMENU_CHOOSENAME_ORBLANK"] 
		end,
		useSUG = true,
...
-------------------------------------buffs/debuffs
	...
	{ -- mainhand
		...
		texttable = absentseconds,
		icon = function() 
		    return GetInventoryItemTexture("player", GetInventorySlotInfo("MainHandSlot")) or "Interface\\Icons\\inv_weapon_shortblade_14" 
		end,
		tcoords = standardtcoords,
...
-------------------------------------stats
	...
	{ -- melee AP
		...
		events = function() 
		    return "UNIT_ATTACK_POWER", "player" 
		end,
		spacebefore = true,
	},
	{ -- melee crit
	...
	{ -- melee haste
		...
		events = function() 
		    return "UNIT_ATTACK_SPEED", "player" 
		end,
	},
	{ -- expertise
	...
	{ -- ranged AP
		...
		events = function() 
		    return "UNIT_RANGED_ATTACK_POWER", "player" 
		end,
		spacebefore = true,
	},
	{ -- range crit
	...
	{ -- range haste
		...
		events = function() 
		    return "UNIT_RANGEDDAMAGE", "player" 
		end,
	},
...
local ConditionsByType = CNDT.ConditionsByType
--[[
...
CNDT:RegisterEvent("PLAYER_LOGIN")
CNDT:PLAYER_LOGIN()
